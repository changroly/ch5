...
line_bot_api = LineBotApi('YZHwUMJOodTEUSgqKxGhXEnH4Q7u7R/HvnlIfKECuX+P/+ZxAu4VxjLwj5CmiwKaAov0hH02FM1p9LTA1zxjUqVtFoQ7LCco0kAayVwDOlHGIMcUA6ERKaQK1FzGDlOnDUtx0zDv7BkqqNHoXjSqGAdB04t89/1O/w1cDnyilFU=')
...
def callback():
    ...
    line_bot_api.reply_message(event.reply_token,TextSendMessage(text='u r at callback'))
    # handle webhook body
    try:
...
def handle_message(event):
    ### 抓到顧客的資料 ###
    line_bot_api.reply_message(event.reply_token,TextSendMessage(text='u r at handle'))
    ...
    if re.match('[0-9]{4}[<>][0-9]',usespeak): # 先判斷是否是使用者要用來存股票的
        ...
        line_bot_api.push_message('U5510bf50c625658f2987a2f52e68defa', TextSendMessage(text='已經刪除成功'))
        return 0
